...
@router.post("/contracts", response_model=AnnotatedContract, tags=["contracts"])
async def upload_contract(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)) -> AnnotatedContract:
    ...
    try:
        ...
        return AnnotatedContract.model_validate(contract)
...
@router.get("/contracts", response_model=list[AnnotatedContract], tags=["contracts"])
async def get_contracts(
    ...
    try:
        ...
        return [AnnotatedContract.model_validate(contract) for contract in contracts]
    except Exception as e:
        logger.error("failed to fetch contracts", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/contracts/{contract_id}", response_model=AnnotatedContract, tags=["contracts"])
...
@router.put("/contracts/{contract_id}/metadata", response_model=AnnotatedContract, tags=["contracts"])
async def update_contract_metadata(contract_id: UUID, metadata: dict, db: AsyncSession = Depends(get_db)) -> AnnotatedContract:
...
@router.put("/contracts/{contract_id}/status", response_model=AnnotatedContract, tags=["contracts"])
async def update_contract_status(contract_id: UUID, status: ContractStatus, db: AsyncSession = Depends(get_db)) -> AnnotatedContract:
